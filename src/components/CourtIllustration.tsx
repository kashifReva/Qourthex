...
function Post({ base, height, id }: { base: [number, number]; height: number; id: string }) {
  return (
    <g>
      <line x1={base[0]} y1={base[1]} x2={base[0]} y2={base[1] - height} stroke="#D4FF00" strokeWidth="2" strokeOpacity="0.85" />
      <circle cx={base[0]} cy={base[1] - height} r="2.4" fill="#D4FF00" />
      <circle cx={base[0]} cy={base[1]} r="2" fill="#D4FF00" fillOpacity="0.6" id={id} />
    </g>
  );
}
...
function GlassWall({ a, b, height, gradId, opacity = 1 }: { a: [number, number]; b: [number, number]; height: number; gradId: string; opacity?: number }) {
  ...
  return (
    <g opacity={opacity}>
      <path d={path} fill={`url(#${gradId})`} stroke="#D4FF00" strokeOpacity="0.4" strokeWidth="1" />
      <line x1={(a[0] + b[0]) / 2} y1={a[1] + (b[1] - a[1]) / 2} x2={(a[0] + b[0]) / 2} y2={a[1] + (b[1] - a[1]) / 2 - height} stroke="#D4FF00" strokeOpacity="0.2" strokeWidth="0.75" />
    </g>
  );
}
    ...
    <svg viewBox="-55 -8 300 200" fill="none">
      <defs>
        ...
        <linearGradient id={`${gid}-floor`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#D4FF00" stopOpacity="0.16" />
      ...
      {/* floor */}
      <path d={floorPath()} fill={`url(#${gid}-floor)`} stroke="#D4FF00" strokeOpacity="0.55" strokeWidth="1.4" />
      <line x1={lerp(fl as unknown as number[], bl as unknown as number[], 0.5)[0]} y1={lerp(fl as unknown as number[], bl as unknown as number[], 0.5)[1]} x2={lerp(fr as unknown as number[], br as unknown as number[], 0.5)[0]} y2={lerp(fr as unknown as number[], br as unknown as number[], 0.5)[1]} stroke="#D4FF00" strokeOpacity="0.3" strokeWidth="1" />
      {/* net */}
      <line
      ...
      {variant === "portable" && (
        <>
          <line x1={fl[0]} y1={fl[1] - 32} x2={fr[0]} y2={fr[1] - 32} stroke="#D4FF00" strokeOpacity="0.5" strokeWidth="1.4" strokeDasharray="3 3" />
          <line x1={bl[0]} y1={bl[1] - 32} x2={br[0]} y2={br[1] - 32} stroke="#D4FF00" strokeOpacity="0.5" strokeWidth="1.4" strokeDasharray="3 3" />
        </>
      )}
